...
export const traversalDescriptions = {
  inorder: {
    ...
    description: 'Returns elements in SORTED order (BST property preserved in treap).',
    useCase: ['Get sorted elements', 'Validate BST property', 'Range queries'],
    complexity: { time: 'O(n)', space: 'O(h)' }
  },
  preorder: {
    name: 'Preorder Traversal',
    order: 'Root -> Left -> Right',
    description: 'Process root first. Shows the priority-based structure of the treap.',
    useCase: ['Copy tree structure', 'Serialize treap', 'Prefix expressions'],
    complexity: { time: 'O(n)', space: 'O(h)' }
  },
  postorder: {
    name: 'Postorder Traversal',
    order: 'Left -> Right -> Root',
    description: 'Process children before root. Useful for deletion operations.',
  ...
  levelorder: {
    ...
    description: 'Visit nodes level by level. Reveals the heap-ordered priority structure.',
    useCase: ['Level-wise printing', 'Verify heap property', 'Serialize for reconstruction'],
...
// Get priority color based on value (higher = more vibrant)
const getPriorityColor = (priority) => {
  const hue = 260; // purple base
  const saturation = 40 + (priority / 100) * 60; // 40-100%
  const lightness = 55 - (priority / 100) * 15; // 55-40%
    ...
    <div className="control-panel">
      ...
      {message && (
        <div className={`message message-${
          message.includes('Error') || message.includes('error') || message.includes('not found') || message.includes('already exists') ? 'error' :
...
// Tree Renderer Component - shows key and priority in each node
export function TreeRenderer({ tree, currentNode, visitedNodes, searchPath, searchResult }) {
  const getNodeStyle = (value) => {
    ...
    return {
      ...
      fontSize: isCurrent || isSearchTarget ? 15 : 13,
  ...
  const renderTree = (node, x, y, level, maxLevel) => {
    ...
    const elements = [];
    const useDefaultColor = style.color === '#6366f1';
    const nodeColor = useDefaultColor ? getPriorityColor(node.priority) : style.color;
    ...
    // Draw node circle with key and priority
    elements.push(
      <g key={`n-${node.value}-${x}`} style={{ filter: style.glow, transition: 'all 0.3s ease' }}>
        <circle cx={x} cy={y} r={style.radius} fill={nodeColor} stroke="white" strokeWidth={style.stroke} style={{ transition: 'all 0.3s ease' }} />
        <text x={x} y={y - 5} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={style.fontSize} fontWeight={style.fontWeight}>
          {node.value}
        </text>
        <text x={x} y={y + 11} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,0.7)" fontSize={9} fontWeight="normal">
          p:{node.priority}
        </text>
      </g>
    );
  ...
  if (!tree) {
    return (
      <div className="tree-empty">
        <p>Treap is empty. Insert some values!</p>
      ...
      {traversalComplete && traversalResult.length > 0 && (
        <div className="traversal-array-section">
          ...
          {traversalType === 'inorder' && (
            <p className="traversal-sorted-note">Notice: The output is sorted! BST property is maintained in the treap.</p>
          )}
