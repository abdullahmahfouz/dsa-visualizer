...
function AVLVisualizer() {
  ...
  const loadTree = async () => {
    try {
      const response = await fetch('/api/avl');
      const data = await response.json();
      setTree(data.tree);
      setSize(data.size || 0);
      setHeight(data.height !== undefined ? data.height : -1);
    } catch (error) {
      console.error('Error loading AVL:', error);
    }
  };
  ...
  const insertNode = async () => {
    ...
    try {
      const response = await fetch('/api/avl/insert', {
        method: 'POST',
      ...
      showMessage(`Inserted ${numValue} (tree rebalanced)`);
      setInsertValue('');
    } catch (error) {
  ...
  const deleteNode = async () => {
    ...
    try {
      const response = await fetch('/api/avl/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ value: numValue })
      });
      ...
      setSize(result.size || 0);
      setHeight(result.height !== undefined ? result.height : -1);
      showMessage(`Deleted ${numValue} (tree rebalanced)`);
      setDeleteValue('');
    } catch (error) {
  ...
  const searchNode = async () => {
    ...
    try {
      const response = await fetch(`/api/avl/search?value=${numValue}`);
      const result = await response.json();
      ...
      if (result.found) {
        ...
        showMessage(`${numValue} not found in AVL Tree`);
      }
    } catch (error) {
  ...
  const clearTree = async () => {
    try {
      await fetch('/api/avl/clear', { method: 'POST' });
      setTree(null);
      setSize(0);
      setHeight(-1);
      resetTraversal();
      clearSearchHighlight();
      showMessage('AVL Tree cleared!');
    } catch (error) {
  ...
  const performTraversal = async (type) => {
    ...
    try {
      const response = await fetch(`/api/avl/${type}`);
      const data = await response.json();
    ...
    <div className="container">
      ...
      <div className="intro-layout">
        <div className="intro-left">
          <ConceptsPanel dataStructure="avl" />
        </div>
        <AIAssistant context="AVL Tree data structure, self-balancing BST, rotations, balance factors, and O(log n) guaranteed operations" />
      </div>
      ...
      <div className="visualizer-layout">
        ...
        <div className="visual-panel">
          ...
          <p className="bst-property-reminder">
            Self-balancing: <code>|Balance Factor| &lt;= 1</code> for all nodes
          </p>
