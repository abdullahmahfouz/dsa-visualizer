...
export function ControlPanel({
  insertValue, buildArray, size, height, max, message, heap, isAnimating,
  ...
  return (
    <div className="control-panel">
      ...
      <div className="heap-info-box max-heap">
        <strong>Priority Queue:</strong> Maximum always at root!
      </div>
      ...
      <div className="operation-group">
        ...
        <div className="btn-group-vertical">
          <button onClick={onExtract} className="btn btn-extract-max" disabled={isAnimating || heap.length === 0}>
            <ArrowUp size={16} /> Extract Max
          </button>
      ...
      <div className="traversal-legend">
        <div className="legend-item"><span className="legend-dot" style={{ background: '#ef4444' }} /><span>Root (Max)</span></div>
      ...
      <div className="info-panel">
        ...
        <div className="info-item"><span className="info-label">Max:</span><span className="info-value">{max ?? '-'}</span></div>
      </div>
  ...
  if (!tree || heap.length === 0) {
    return (
      <div className="tree-empty">
        <p>Max Heap is empty. Insert some values!</p>
        <p className="tree-empty-hint">Try: 5, 3, 8, 1, 9 or build from array</p>
      </div>
    );
  }
      ...
      <div className="heap-array">
        {heap.map((value, index) => {
          ...
          return (
            <div
              key={index}
              className={`heap-array-cell ${isHighlighted || isSwapping ? 'highlighted' : ''} ${isRoot ? 'root-max' : ''}`}
            >
